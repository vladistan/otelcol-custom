...
func newProcessor(logger *zap.Logger, config *Config, nextConsumer consumer.Logs) *kafkaLogProcessor {
...
func (p *kafkaLogProcessor) Capabilities() consumer.Capabilities {
	return consumer.Capabilities{MutatesData: true}
}

func (p *kafkaLogProcessor) Start(_ context.Context, _ component.Host) error {
	p.logger.Info("Kafka log processor started",
		zap.String("service_name", p.config.ServiceName),
		zap.Bool("rewrite_messages", p.config.RewriteMessages),
		zap.Bool("extract_logger", p.config.ExtractLogger),
		zap.Bool("adjust_severity", p.config.AdjustSeverity),
	)
	return nil
}

func (p *kafkaLogProcessor) Shutdown(_ context.Context) error {
	return nil
}

func (p *kafkaLogProcessor) ConsumeLogs(ctx context.Context, ld plog.Logs) error {
...
func (p *kafkaLogProcessor) matchesServiceName(resourceAttrs pcommon.Map) bool {
...
func (p *kafkaLogProcessor) processLogRecord(lr plog.LogRecord) {
	// Get body as string
	var body string
	...
	// Check if this looks like a Kafka log
	if !IsKafkaLog(body) {
		return
	}

	// Parse the log
	info := ParseKafkaLog(body)
	if info == nil {
		return
	}
	...
	if p.config.ExtractLogger && info.Logger != "" {
		attrs.PutStr("kafka.logger", info.Logger)
	}
